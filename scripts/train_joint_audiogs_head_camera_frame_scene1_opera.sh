...
CONFIG="${ROOT}/configs/scene1_opera_b_joint_av_audiogs_head_camera_frame.yaml"
RUN_DIR="${ROOT}/runs/scene1_opera_b_joint_av_audiogs_head_camera_frame"
...
if [[ -n "${JOINT_STEPS_OVERRIDE:-}" ]]; then
  ...
  echo "AudioGS camera-frame Route B scene1_opera joint_steps=${JOINT_STEPS_OVERRIDE} (override)"
else
  echo "AudioGS camera-frame Route B scene1_opera using config joint_steps"
fi
