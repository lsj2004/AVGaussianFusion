...
RUN_DIR="${ROOT}/runs/scene7_playing_300_c_soft_av_gaussians_mid_side_phase"
MANIFEST="${ROOT}/runs/scene7_playing_300_a/scene_manifest.json"
CHECKPOINT="${RUN_DIR}/soft_av.pt"

if [[ ! -f "${CHECKPOINT}" ]]; then
  echo "missing Route C mid-side phase checkpoint: ${CHECKPOINT}" >&2
  echo "run scripts/train_soft_av_scene7_playing_300_mid_side_phase.sh first" >&2
  exit 1
fi
