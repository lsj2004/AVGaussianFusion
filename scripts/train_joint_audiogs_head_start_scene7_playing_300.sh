...
CONFIG="${ROOT}/configs/scene7_playing_300_b_joint_av_audiogs_head_start.yaml"
RUN_DIR="${ROOT}/runs/scene7_playing_300_b_joint_av_audiogs_head_start"
...
if [[ ! -f "${CONFIG}" ]]; then
  echo "missing start-anchored AudioGS-head config: ${CONFIG}" >&2
  exit 1
fi
...
if [[ -n "${JOINT_STEPS_OVERRIDE:-}" ]]; then
  ...
  echo "AudioGS start-anchored Route B scene7_playing_300 joint_steps=${JOINT_STEPS_OVERRIDE} (override)"
else
  echo "AudioGS start-anchored Route B scene7_playing_300 using config joint_steps"
fi
